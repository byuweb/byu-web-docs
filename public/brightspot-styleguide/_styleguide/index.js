...
$__System.register("5", ["2", "3", "4", "6"], function($__export) {
  "use strict";
  var Prism,
      PrismJson,
      PrismMarkdown,
  ...
  return {
    setters: [function($__m) {
      ...
      PrismJson = $__m.default;
    }, function($__m) {
      PrismMarkdown = $__m.default;
    }, function($__m) {
      Util = $__m.default;
    }],
    execute: function() {
      TabbedContent = function() {
        function TabbedContent(ctx) {
        ...
        return ($traceurRuntime.createClass)(TabbedContent, {
          get selectors() {
            return this.settings.selectors;
          },
          init: function() {
            var self = this;
            var exampleFrame = document.querySelector(("." + this.selectors.iframeContent));
            if (exampleFrame) {
              exampleFrame.addEventListener('load', function(event) {
                if (self.settings.prismMap[self.dataType] !== undefined) {
                  ...
                  if (prismElement) {
                    ...
                    var cssAppend = document.querySelector('link[href="/_styleguide/index.css"]').cloneNode();
          ...
          updateTitle: function(element) {
            var contentTitle = document.querySelector(("." + this.selectors.content + "-title"));
            if (contentTitle !== null) {
              contentTitle.parentNode.removeChild(contentTitle);
            }
            var heading = document.createElement('h1');
            heading.className = (this.selectors.content + "-title");
            heading.insertAdjacentText('afterbegin', element.text);
            document.querySelector(("." + this.selectors.content)).insertAdjacentElement('afterbegin', heading);
          },
          removeTitle: function() {
            var title = document.querySelector(("." + this.selectors.content + "-title"));
            if (title) {
              title.remove();
            }
          },
          updateTabs: function(element) {
            var $__4 = this;
            var dataSources = JSON.parse(element.getAttribute('data-source'));
            var baseURL = element.getAttribute('href').split('.html');
            var $tabList = document.querySelector(("." + this.selectors.tabList));
            var self = this;
            if (!$tabList) {
              $tabList = document.createElement('ul');
              $tabList.className = this.selectors.tabList;
              document.querySelector(("." + this.selectors.example)).insertAdjacentElement('beforebegin', $tabList);
              document.querySelector(("." + this.selectors.tabList)).addEventListener('Styleguide:tabsUpdate', function(evt) {
                var tabs = evt.currentTarget;
                var hashTab = window.location.hash;
                if (hashTab !== '') {
                  tabs.querySelector('[name=' + hashTab.replace('#', '') + ']').click();
                } else {
                  tabs.querySelector('a').click();
                }
              });
              $tabList = document.querySelector(("." + this.selectors.tabList));
            }
            if (!dataSources) {
              return false;
            }
            ...
            document.querySelector(("." + this.selectors.tabList)).removeAttribute("data-initialized");
            this.dataType = dataSources[Object.keys(dataSources)[0]];
            for (var key in dataSources) {
              if (dataSources.hasOwnProperty(key)) {
                var dataSource = dataSources[key];
                var text = dataSource.text || dataSource;
                var iframeSrc = dataSource.src || (baseURL[0] + "." + (dataSource.ext || key));
                var index = Object.keys(dataSources).indexOf(key);
                var tabItem = document.createElement('li');
                tabItem.className = (self.selectors.tabList + "-item");
                var tabLink = document.createElement('a');
                tabLink.href = iframeSrc;
                tabLink.textContent = text;
                tabLink.target = self.selectors.iframeContent;
                tabLink.name = key;
                tabLink.className = (self.selectors.tabList + "-link");
                tabLink.addEventListener('click', function(evt) {
                  var link = evt.currentTarget;
                  var activeTab = link.parentNode;
                  var tabs = $traceurRuntime.spread($tabList.querySelectorAll('li'));
                  tabs.forEach(function(tab) {
                    tab.removeAttribute('data-active');
                  });
                  activeTab.setAttribute('data-active', '');
                  $__4.dataType = link.name;
                  window.history.replaceState({}, link.getAttribute('title'), ("#" + $__4.dataType));
                });
                tabItem.insertAdjacentElement('afterbegin', tabLink);
                if (index === 0) {
                  tabItem.setAttribute('data-active', '');
                }
                document.querySelector(("." + this.selectors.tabList)).insertAdjacentElement('beforeend', tabItem);
                document.querySelector(("." + this.selectors.tabList)).setAttribute("data-initialized", "");
              }
            }
            var tabCreationEvent = document.createEvent('Event');
            tabCreationEvent.initEvent('Styleguide:tabsUpdate', false, true);
            document.querySelector(("." + this.selectors.tabList)).dispatchEvent(tabCreationEvent);
          },
          removeTabs: function() {
            var tabs = document.querySelector(("." + this.selectors.tabList));
            if (tabs) {
              tabs.remove();
            }
          }
      ...
      $__export("TabbedContent", TabbedContent);
    }
  };
});
...
(function webpackUniversalModuleDefinition(root, factory) {
  if (typeof exports === 'object' && typeof module === 'object')
    ...
    define("7", [], factory) && define("Fuse", ["7"], function(m) {
      return m;
    });
  ...
  return (function(modules) {
    ...
    module.exports = function(text, pattern) {
      ...
      if (isMatch) {
        for (var i = 0,
            matchesLen = matches.length; i < matchesLen; i += 1) {
          var match = matches[i];
    ...
    var Fuse = function() {
      ...
      _createClass(Fuse, [{
        ...
        value: function _search() {
          ...
          var weights = {};
          for (var _i = 0,
              _len = list.length; _i < _len; _i += 1) {
            var item = list[_i];
            for (var j = 0,
                keysLen = this.options.keys.length; j < keysLen; j += 1) {
        ...
        value: function _analyze(_ref2, _ref3) {
          ...
          if (typeof value === 'string') {
            ...
            for (var _i3 = 0,
                len = value.length; _i3 < len; _i3 += 1) {
              this._analyze({
                key: key,
                value: value[_i3],
                record: record,
        ...
        value: function _format(results) {
          ...
          if (this.options.includeMatches) {
            transformers.push(function(result, data) {
              ...
              data.matches = [];
              for (var i = 0,
                  len = output.length; i < len; i += 1) {
                var item = output[i];
                var obj = {indices: item.matchedIndices};
                if (item.key) {
                  obj.key = item.key;
                }
          ...
          for (var i = 0,
            ...
            for (var j = 0,
                _len2 = transformers.length; j < _len2; j += 1) {
...
$__System.register("8", ["7", "6"], function($__export) {
  "use strict";
  var Fuse,
      Util,
      Navigation;
    ...
    execute: function() {
      Navigation = function() {
        function Navigation(ctx) {
          ...
          this.$navigationLinks = $traceurRuntime.spread(document.querySelectorAll(("." + this.selectors.groups + "-examples-link")));
        ...
        return ($traceurRuntime.createClass)(Navigation, {
          ...
          get navigationStateStorage() {
            return window.localStorage.getItem('navigation-state');
          },
          set navigationStateStorage(navState) {
            window.localStorage.setItem('navigation-state', navState);
          },
          addfilterAttributes: function(filtersArray) {
            var $navigationContent = document.querySelector(("." + this.selectors.navigation + "-content"));
          ...
          init: function(app) {
            ...
            var exampleLinkIndex = 0;
            var fuse = new Fuse(this.navList, this.settings.filterOptions);
            var navigationElements = $traceurRuntime.spread(document.querySelectorAll(("." + self.selectors.dropdown + "-menu, ." + self.selectors.dropdown + "-toggle")));
            navigationElements.forEach(function($item) {
              $item.addEventListener('click', function(event) {
                event.stopPropagation();
              });
            });
            var navigationItems = $traceurRuntime.spread(document.querySelectorAll(("." + this.selectors.groups + "-name, ." + this.selectors.groups + "-examples-link")));
            navigationItems.forEach(function($item, index, array) {
              var item = {};
              if ($item.classList.value === 'StyleguideGroups-name') {
                ...
                $__5.navList.push(item);
              }
            });
            var searchObject = Util.locationSearchToObject(window.location.search);
            var navigationLinks = $traceurRuntime.spread(document.querySelectorAll(("." + this.selectors.groups + "-examples-link")));
            navigationLinks.forEach(function(element) {
              if (element.pathname === searchObject['file']) {
                element.setAttribute('data-active', '');
                app.update(element);
                !element.dataset.tabbed ? element.click() : null;
              }
              element.addEventListener('click', function(event) {
                window.history.replaceState({}, this.getAttribute('title'), '?file=' + this.pathname);
                var $activeNavItem = document.querySelector(("." + self.selectors.groups + "-examples-link[data-active]"));
                if ($activeNavItem) {
                  $activeNavItem.removeAttribute('data-active');
                }
                if (event.target.classList.contains("icon-external-link")) {
            ...
            if (searchObject['filters']) {
              this.filterList = searchObject['filters'].split(',');
              this.filterList.forEach(function(filerName) {
                var $inputFilter = document.querySelector(("[name=filter-" + filerName + "]"));
                if ($inputFilter) {
            ...
            var navigationStorage = this.navigationStateStorage;
            if (navigationStorage === null) {
              this.navigationStateStorage = '';
            }
            if (this.navigationStateStorage.length > 0) {
              this.navigationState = navigationStorage.split(',');
              this.ctx.querySelectorAll(("." + this.selectors.dropdown + "-button, ." + this.selectors.groups + "-name")).forEach(function(element) {
                if ($__5.navigationState.indexOf(element.childNodes[0].nodeValue) > -1) {
            ...
            this.ctx.querySelector(("." + this.selectors.dropdown + "-search-clear")).addEventListener('click', function(event) {
              var $searchInput = document.querySelector(("." + self.selectors.dropdown + "-search-input"));
              $searchInput.value = '';
              $searchInput.focus();
              self.clearListItems(document.querySelector(("." + self.selectors.dropdown + "-results")));
            });
            this.ctx.querySelector(("." + this.selectors.dropdown + "-search-input")).addEventListener('click', function(event) {
              self.createSearchList(this);
            });
            this.ctx.querySelector(("." + this.selectors.dropdown + "-search-input")).addEventListener('keyup', function(event) {
              var $__6 = this;
              var $resultList = document.querySelector(("." + self.selectors.dropdown + "-results"));
              var $resultItems = $traceurRuntime.spread(document.querySelectorAll(("." + self.selectors.dropdown + "-result")));
              if (event.keyCode === 13) {
              ...
              results.forEach(function(result, index, array) {
                var resultName = result.item.name;
                var resultGroupName = result.item.groupName;
                result.matches.forEach(function(item) {
                  if (item.key === self.settings.filterOptions.keys[0].name) {
                    resultName = self.highlightSearchKey(result.item.name, item.indices);
                  }
                  if (item.key === self.settings.filterOptions.keys[1].name) {
                    resultGroupName = self.highlightSearchKey(result.item.groupName, item.indices);
                  }
                });
                var listItemObj = document.createElement('li');
                listItemObj.className = (self.selectors.dropdown + "-result");
                listItemObj.insertAdjacentHTML('afterbegin', resultGroupName ? (resultName + " <div class='" + self.selectors.dropdown + "-result-group'>" + resultGroupName + "</div>") : ("" + resultName));
                listItemObj.setAttribute('data-index', result.item.index);
                listItemObj.setAttribute('data-score', result.score);
                listItemObj.setAttribute('data-original', result.item.name);
                listItemObj.addEventListener('click', function(evt) {
                  $__6.parentNode.querySelector('[data-active]').removeAttribute('data-active');
                  $__6.setAttribute('data-active', '');
                  self.searchResultsIndex = index;
                  self.$navigationLinks[result.item.index].click();
                  $searchInput.value = result.item.name;
                  $resultList.style.display = 'none';
                  var topNav = document.querySelector(("." + self.selectors.groups + "-examples-link[data-active]")).documentOffsetTop() - document.querySelector(("." + self.selectors.navigation + "-group")).clientHeight / 2;
                  document.querySelector(("." + self.selectors.navigation + "-group")).scrollTop = topNav;
                });
                if (index === 0) {
                  listItemObj.setAttribute('data-active', '');
                }
                $resultList.insertAdjacentElement('beforeend', listItemObj);
              });
            });
            var sectionHeadings = $traceurRuntime.spread(document.querySelectorAll(("." + this.selectors.groups + "-name")));
            sectionHeadings.forEach(function($item) {
              $item.addEventListener('click', function() {
                self.updateNavigationState(self.navigationState, this.childNodes[0].nodeValue, this.parentNode);
              });
            });
            var $dropdownToggle = document.querySelector(("." + this.selectors.dropdown + "-toggle"));
            if ($dropdownToggle) {
              $dropdownToggle.addEventListener('click', function(event) {
                self.toggleMenu(this.parentNode);
              });
            }
            var filters = $traceurRuntime.spread(document.querySelectorAll(("." + this.selectors.dropdown + "-filter input[type=checkbox]")));
            filters.forEach(function($item) {
              $item.addEventListener('click', function(event) {
                self.filterList = Util.updateArray(self.filterList, this.value);
                var baseUrl = Util.updateSearchURL(window.location, {filters: self.filterList});
                window.history.pushState({}, ("Filters:" + self.filterList), baseUrl);
                self.addfilterAttributes(self.filterList);
              });
            });
            var $navigationButton = document.querySelector(("." + this.selectors.navigation + "-button"));
            if ($navigationButton) {
              $navigationButton.addEventListener('click', function() {
                var navFlag = document.querySelector('body').getAttribute('data-nav');
                if (navFlag === '') {
                  document.querySelector('body').removeAttribute('data-nav');
                } else {
                  document.querySelector('body').setAttribute('data-nav', '');
                }
              });
            }
          },
          highlightSearchKey: function(text, matches) {
            var result = [];
          ...
          createSearchList: function(element) {
            var $results = document.querySelector(("." + this.selectors.dropdown + "-results"));
            if (!$results) {
              var $list = document.createElement('ul');
              $list.className = (this.selectors.dropdown + "-results");
              element.insertAdjacentElement('afterend', $list);
            } else {
              $results.removeAttribute('style');
            }
          },
          stepThroughList: function($resultItems, direction) {
            ...
            $resultItems[this.searchResultsIndex].scrollIntoView(false);
          },
          clearListItems: function($resultList) {
            if ($resultList) {
          ...
          toggleMenu: function($ctxElement) {
          ...
          updateNavigationState: function(navigationArray, newValue, $menuNode) {
            navigationArray = Util.updateArray(navigationArray, newValue);
            this.navigationStateStorage = navigationArray;
            this.toggleMenu($menuNode);
          }
        }, {});
      }();
      $__export("Navigation", Navigation);
    }
  };
});
...
$__System.registerDynamic('6', [], true, function ($__require, exports, module) {
  ...
  var Util = {
    ...
    getImageSrcSetAttributeValidationRules() {
      const rules = [['validDescriptor', {
        description: name => `One or more of the image candidate strings for ${name} does not contain a 'w' or an 'x' character.`
      }], ['doubleDescriptorViolation', {
        description: name => `The image candidate strings for ${name} are not valid. All candidate strings must be of the same type (e.g. all w or all x).`
      }], ['widthDescriptorFormatViolation', {
        description: name => `One or more of the image candidate strings for ${name} are not valid. A width descriptor should consist of: an optional space character, a valid non-negative integer giving a number greater than zero representing the width descriptor value, and a U+0077 LATIN SMALL LETTER W character.`,
        regex: /^\s?[1-9]\d*w$/
      }], ['pixelDensityDescriptorFormatViolation', {
        description: name => `One or more of the image candidate strings for ${name} are not valid. A pixel density descriptor should consist of: an optional space character, a valid floating-point number giving a number greater than zero representing the pixel density descriptor value, and a U+0078 LATIN SMALL LETTER X character.`,
        regex: /(^\s?(0?\.\d+)x$)|(^\s?[1-9]+\d*((\.\d+)?))x$/
      }], ['sameDescriptorValueViolations', {
        description: name => `The image candidate strings for ${name} are not valid. All descriptor values must be unique.`
      }], ['source', 'http://w3c.github.io/html/semantics-embedded-content.html#valid-source-size-list']];
...
$__System.register("a", ["9"], function($__export) {
  "use strict";
  var Storage,
      THEME_STORAGE_KEY,
      syncOverrides;
  function receiveMessage(event) {
    var exampleFrame = document.querySelector('iframe.StyleguideExample-frame');
    var message = event.data;
    var _setCSSProp = function(prop, val) {
      exampleFrame.contentDocument.documentElement.style.setProperty(prop, val);
    };
    switch (message._type) {
      case 'BSP.theme.overrides':
        Storage.set(THEME_STORAGE_KEY, JSON.stringify(message));
        Object.keys(message.data).forEach(function(key) {
          var value = message.data[key];
          if (key.includes('/theme.') && value !== '') {
            var variableName = ("--" + key.slice(key.lastIndexOf('.') + 1));
            _setCSSProp(variableName, value);
          }
        });
        break;
    }
    exampleFrame.contentWindow.postMessage(message, '*');
  }
  return {
    setters: [function($__m) {
      Storage = $__m.default;
    ...
    execute: function() {
      THEME_STORAGE_KEY = 'BSP.ThemeBundle.overrides';
      syncOverrides = function() {
        var exampleFrame = document.querySelector('iframe.StyleguideExample-frame');
        exampleFrame.onload = function(event) {
          var overrides = Storage.get(THEME_STORAGE_KEY);
          overrides = overrides ? JSON.parse(overrides) : {};
          if (Object.keys(overrides).length) {
            Object.keys(overrides.data).forEach(function(key) {
              if (key.includes('/theme.')) {
                var variableName = ("--" + key.slice(key.lastIndexOf('.') + 1));
                exampleFrame.contentDocument.documentElement.style.setProperty(variableName, overrides.data[key]);
              }
            });
            exampleFrame.contentWindow.postMessage(overrides, '*');
          }
        };
      };
      $__export('default', {connect: function() {
...
$__System.register("1", ["2", "3", "4", "5", "8", "6", "a"], function($__export) {
  "use strict";
  var Prism,
      PrismJson,
      PrismMarkdown,
  ...
  return {
    setters: [function($__m) {
      ...
      TabbedContent = $__m.TabbedContent;
    }, function($__m) {
      Navigation = $__m.Navigation;
    }, function($__m) {
      Util = $__m.default;
    }, function($__m) {
      ExampleBridge = $__m.default;
    }],
    execute: function() {
      document.onreadystatechange = function() {
        if (document.readyState === 'interactive') {
          var application = {update: function(element) {
              if (element.dataset.tabbed) {
                tabbedContent.updateTitle(element);
                tabbedContent.updateTabs(element);
              } else {
                tabbedContent.removeTitle(element);
                tabbedContent.removeTabs(element);
              }
            }};
          var tabbedContent = new TabbedContent(document.querySelector('.StyleguideContent'), {});
          tabbedContent.init();
          var navigation = new Navigation(document.querySelector('.StyleguideNavigation'), {});
          navigation.init(application);
          var copyableElements = $traceurRuntime.spread(document.querySelectorAll("[data-copyable]"));
